...
import {
  ...
  Alert, Chip, LinearProgress, Tabs, Tab,
} from '@mui/material';
import {
  Inventory, Warning, Assignment, TrendingUp, Security,
  People, Pending, Receipt, Schedule, Refresh, CheckCircle,
  ...
  return (
    <Box>
      ...
      <Alert
        ...
        action={<Button color="inherit" size="small" startIcon={<Refresh />} onClick={() => navigate('/admin/reference')}>Check Now</Button>}
      >
          ...
          <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)', height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Inventory sx={{ color: '#3b82f6', fontSize: 40 }} />
              </Box>
          ...
          <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)', height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <People sx={{ color: '#10b981', fontSize: 40 }} />
              </Box>
            </CardContent>
          </Card>
        </Grid>
          ...
          <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)', height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Schedule sx={{ color: '#3b82f6', fontSize: 40 }} />
              </Box>
            </CardContent>
          </Card>
        </Grid>
          ...
          <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)', height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Assignment sx={{ color: '#8b5cf6', fontSize: 40 }} />
              </Box>
              <Button size="small" sx={{ mt: 1 }} onClick={() => navigate('/admin/requests')}>Review</Button>
            </CardContent>
          </Card>
        </Grid>
          ...
          <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)', height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Receipt sx={{ color: '#ef4444', fontSize: 40 }} />
              </Box>
            </CardContent>
          </Card>
        </Grid>
          ...
          <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)', height: '100%' }}>
            <CardContent sx={{ p: 3 }}>
              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <Pending sx={{ color: '#eab308', fontSize: 40 }} />
              </Box>
              <Button size="small" sx={{ mt: 1 }} onClick={() => navigate('/admin/calibrations')}>Review</Button>
            </CardContent>
          </Card>
        </Grid>
      </Grid>

      {/* Charts - Two Separate Full-Width Rows */}
      <Grid container spacing={3} sx={{ mb: 6 }}>
        {/* Row 1: Monthly Calibration Trend - Full Width */}
        <Grid item xs={12}>
          <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
            <CardContent>
              <Typography variant="h6" fontWeight={600} gutterBottom>Monthly Calibration Trend</Typography>
              <Box sx={{ height: 380 }}>
        ...
        {/* Row 2: Calibration Status Distribution - Full Width */}
        <Grid item xs={12}>
          <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
            <CardContent>
      ...
      <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)', mb: 6 }}>
        <CardContent sx={{ p: 3 }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
            <Button variant="contained" startIcon={<People />} onClick={() => navigate('/admin/users')}>ADD USER</Button>
            <Button variant="contained" startIcon={<Assignment />} onClick={() => navigate('/admin/requests')}>REVIEW REQUESTS</Button>
            <Button variant="contained" startIcon={<TrendingUp />} onClick={() => navigate('/admin/reports')}>EXPORT REPORT</Button>
            <Button variant="contained" startIcon={<CheckCircle />} onClick={() => navigate('/admin/calibrations')}>APPROVE CALIBRATIONS</Button>
          </Box>
        </CardContent>
      </Card>
                  ...
                  <TableBody>
                    {pendingApprovals.map((row) => (
                      <TableRow key={row.id} hover>
                        ...
                        <TableCell><Chip label={row.type} size="small" color={row.type === 'Calibration' ? 'primary' : 'secondary'} /></TableCell>
                        <TableCell>{row.equipment || row.client}</TableCell>
                        <TableCell>{row.staff || row.client}</TableCell>
                        <TableCell>{row.date}</TableCell>
                        <TableCell align="center">
                          <Button variant="contained" color="success" size="small" sx={{ mr: 1 }} onClick={() => navigate('/admin/calibrations')}>
                            Approve
                          </Button>
                          <Button variant="outlined" color="error" size="small" onClick={() => navigate('/admin/calibrations')}>
                            Reject
                          </Button>
                        </TableCell>
                      </TableRow>
      ...
      <Card sx={{ borderRadius: 3, boxShadow: '0 10px 30px rgba(0,0,0,0.08)' }}>
        <CardContent sx={{ p: 4 }}>
          ...
          <Button variant="outlined" startIcon={<Security />} sx={{ mt: 2 }} onClick={() => navigate('/admin/reference')}>
            Run Verification
          </Button>
        </CardContent>
      </Card>
